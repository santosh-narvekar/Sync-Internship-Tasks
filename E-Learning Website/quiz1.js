...
let registeredUsername = currentAccount.split('"')[3];
...
options.forEach((s, _) => {
  s.addEventListener("click", function () {
    ...
    basicPrgAns.push(s.textContent);
    let userChoice = basicPrgAns.pop();

    if (userChoice) {
      let index = basicPrgAns.indexOf(userChoice);
      if (index in basicPrgAns && index != -1) {
        Corrects++;
